...
func EVMProvider(db *sqlx.DB, chain evm.Chain, lggr logger.Logger, spec job.Job, pr pipeline.Runner, ks keystore.Eth) (evmrelay.OCR2KeeperProvider, error) {
...
func EVMDependencies20(
	...
	if keeperProvider, err = EVMProvider(db, chain, lggr, spec, pr, ks); err != nil {
