...
func NewStream(lggr logger.Logger, id relayllo.StreamID, runResults chan<- *pipeline.Run, jb job.Job, spec pipeline.Spec, runner Runner) Stream {
	return newStream(lggr, id, runResults, jb, spec, runner)
}

func newStream(lggr logger.Logger, id relayllo.StreamID, runResults chan<- *pipeline.Run, jb job.Job, spec pipeline.Spec, runner Runner) *stream {
	return &stream{id, lggr, runResults, jb, spec, runner}
}
...
func (s *stream) executeRun(ctx context.Context) (*pipeline.Run, pipeline.TaskRunResults, error) {
	vars := pipeline.NewVarsFrom(map[string]interface{}{
		"jb": map[string]interface{}{
			"databaseID":    s.jb.ID,
			"externalJobID": s.jb.ExternalJobID,
			"name":          s.jb.Name.ValueOrZero(),
		},
	})
